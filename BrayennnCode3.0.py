...
def brayen_dump():
	...
	try:
		...
		koH = requests.get("https://graph.facebook.com/+pil+".format(5000),params = tokenku[5000],cookies = {'cookie': kukis}).json()
		for pi in koH['friends']['data']:
			try:id.append(pi['id']+'|'+pi['name'])
			except:continue
		print(f' [+] Total ID Yang Terkumpul : {h}'+str(len(id)));setting()
	except requests.exceptions.ConnectionError:
